...
local fpsPingScript = nil

secFpsPing:Toggle({ Title = "Fps/Ping Counter", Type = "Checkbox", Flag = "fpsPingOn", Default = false,
    Callback = function(on)
        if on then
            pcall(function()
                fpsPingScript = loadstring(game:HttpGet("https://raw.githubusercontent.com/bezenadduca-code/Jjs/refs/heads/main/Fps/Ping%20counter"))()
            end)
        else
            if fpsPingScript then
                pcall(function()
                    if typeof(fpsPingScript) == "Instance" then
                        fpsPingScript:Destroy()
                    end
                end)
                fpsPingScript = nil
            end
            -- destroy any leftover ScreenGui from the script
            pcall(function()
                for _, v in ipairs(lp.PlayerGui:GetChildren()) do
                    if v.Name == "FpsPingCounter" or v.Name == "FPSCounter" or v.Name == "PingCounter" then
                        v:Destroy()
                    end
                end
            end)
