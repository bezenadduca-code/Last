...
local function itemTPGetItems()
    ...
    for _, obj in ipairs(svc.WS:GetDescendants()) do
        local n = obj.Name
        if (n == "BloxyCola" or n == "Medkit") and not seen[obj] then
            local held = false
            for _, plr in ipairs(svc.Players:GetPlayers()) do
                local ch = plr.Character
                local bp = plr:FindFirstChildOfClass("Backpack")
                if (ch and obj:IsDescendantOf(ch)) or (bp and obj:IsDescendantOf(bp)) then
                    held = true; break
                end
            end
            if not held then
    ...
    pcall(function()
        ...
        for _, item in ipairs(items) do
            pcall(function()
                ...
                hrp.CFrame = item.part.CFrame * CFrame.new(0, 2, 0)
                task.wait(0.12)

                -- fire ALL ProximityPrompts found on the item
                for _, desc in ipairs(item.obj:GetDescendants()) do
                    if desc:IsA("ProximityPrompt") then
                        pcall(function() fireproximityprompt(desc) end)
                    end
                end
                local rootPrompt = item.obj:FindFirstChildOfClass("ProximityPrompt")
                if rootPrompt then
                    pcall(function() fireproximityprompt(rootPrompt) end)
                end

                -- if it's a Tool sitting in workspace, equip it directly
                if item.obj:IsA("Tool") and item.obj.Parent == svc.WS then
                    pcall(function() hum:EquipTool(item.obj) end)
                end
...
-- FPS / Ping Counter
------------------------------------------------------------------------
local secPerf = tabInterface:Section({ Title = "FPS / Ping Counter", Opened = true })

local perfGui    = nil
local perfThread = nil
local perfOn     = false

local perfRenderConn = nil

local function perfCreate()
    pcall(function()
        if perfGui and perfGui.Parent then return end
        local pg = lp:FindFirstChildOfClass("PlayerGui"); if not pg then return end

        perfGui = Instance.new("ScreenGui")
        perfGui.Name            = "SAKIPerfGui"
        perfGui.ResetOnSpawn    = false
        perfGui.DisplayOrder    = 15
        perfGui.ZIndexBehavior  = Enum.ZIndexBehavior.Sibling
        perfGui.Parent          = pg

        local fpsLbl = Instance.new("TextLabel")
        fpsLbl.Name                   = "FPSLabel"
        fpsLbl.Size                   = UDim2.new(0, 120, 0, 22)
        fpsLbl.Position               = UDim2.new(0, 310, 0, 160)
        fpsLbl.BackgroundTransparency = 1
        fpsLbl.TextSize               = 14
        fpsLbl.Font                   = Enum.Font.SourceSansBold
        fpsLbl.Text                   = "FPS: --"
        fpsLbl.TextColor3             = Color3.fromRGB(0, 200, 0)
        fpsLbl.TextXAlignment         = Enum.TextXAlignment.Left
        fpsLbl.TextStrokeTransparency = 0.5
        fpsLbl.TextStrokeColor3       = Color3.new(0, 0, 0)
        fpsLbl.Parent                 = perfGui

        local pingLbl = Instance.new("TextLabel")
        pingLbl.Name                   = "PingLabel"
        pingLbl.Size                   = UDim2.new(0, 120, 0, 22)
        pingLbl.Position               = UDim2.new(0, 310, 0, 183)
        pingLbl.BackgroundTransparency = 1
        pingLbl.TextSize               = 14
        pingLbl.Font                   = Enum.Font.SourceSansBold
        pingLbl.Text                   = "Ping: --"
        pingLbl.TextColor3             = Color3.fromRGB(0, 200, 0)
        pingLbl.TextXAlignment         = Enum.TextXAlignment.Left
        pingLbl.TextStrokeTransparency = 0.5
        pingLbl.TextStrokeColor3       = Color3.new(0, 0, 0)
        pingLbl.Parent                 = perfGui

        -- FPS via Heartbeat (works in executor context, unlike RenderStepped)
        local frames = 0
        local lastT  = tick()
        if perfRenderConn then pcall(function() perfRenderConn:Disconnect() end) end
        perfRenderConn = svc.Run.Heartbeat:Connect(function()
            frames = frames + 1
            local now = tick()
            if now - lastT >= 1 then
            ...
            while perfOn do
                pcall(function()
                    ...
                    if pingLbl and pingLbl.Parent then
                        ...
                        pingLbl.TextColor3 =
                            ping < 100  and Color3.fromRGB(0, 200, 0)   or
                            ping < 200  and Color3.fromRGB(255, 165, 0) or
...
local function perfDestroy()
    perfOn = false
    if perfRenderConn then pcall(function() perfRenderConn:Disconnect() end); perfRenderConn = nil end
    if perfThread then task.cancel(perfThread); perfThread = nil end
    if perfGui and perfGui.Parent then perfGui:Destroy(); perfGui = nil end
end

secPerf:Toggle({ Title = "Show FPS / Ping", Type = "Checkbox", Flag = "perfOn", Default = false,
    Callback = function(on)
        perfOn = on
        if on then perfCreate() else perfDestroy() end
    end
})
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        -- FPS/Ping counter
        perfOn = win.Flags.perfOn or false
        if perfOn then perfCreate() end
